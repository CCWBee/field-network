  ...
  if (badges.length === 0) {
    return (
      <div className={`rounded-sm border border-dashed border-ink-300 p-6 text-center ${className}`}>
        ...
        <p className="text-sm text-ink-500">No badges earned yet</p>
        <p className="text-xs text-ink-300 mt-1">Complete tasks and maintain reliability to earn badges</p>
      </div>
    );
  }
  ...
  return (
    <div className={className}>
      <div className={`grid ${classes.grid}`}>
        {displayBadges.map((badge, index) => {
          ...
          return (
            <div
              ...
              className={`${classes.card} rounded-sm border ${colors.border} ${colors.bg} flex items-start gap-3`}
            >
              <BadgeIcon
              ...
              <div className="flex-1 min-w-0">
                ...
                {size !== 'sm' && (
                  <p className={`text-ink-500 line-clamp-2 ${classes.desc}`}>
                    {badge.description}
                  </p>
                )}
                <div className="flex items-center gap-2 mt-1">
                  <span className={`${classes.tier} px-1.5 py-0.5 rounded-sm border ${colors.border} ${colors.text} capitalize`}>
                    {badge.tier}
                  </span>
                  {badge.category && size === 'lg' && (
                    <span className={`${classes.tier} text-ink-500 capitalize`}>
                      {badge.category}
                    </span>
                  )}
                </div>
        ...
        {hasMore && (
          <div className={`${classes.card} rounded-sm border border-dashed border-ink-300 flex items-center justify-center text-ink-500`}>
