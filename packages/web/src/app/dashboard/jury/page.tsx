...
export default function JuryPoolPage() {
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-ink-900 tracking-tight mb-1">Jury Duty</h1>
      ...
      {/* Info Banner */}
      <div className="bg-signal-blue/10 rounded-sm border border-signal-blue/30 p-4 mb-6">
        <div className="flex items-start gap-3">
          <svg className="w-5 h-5 text-signal-blue mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {disputes.length === 0 ? (
        <div className="bg-paper rounded-sm border border-ink-200 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-ink-700 mb-1">No Active Jury Duties</h3>
        ...
        <div className="space-y-4">
          {disputes.map((dispute) => (
            <Link
              ...
              className="block bg-paper rounded-sm border border-ink-200 hover:border-field-500/30 transition-colors"
            >
              <div className="p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      {dispute.hasVoted ? (
                        <span className="px-2 py-1 text-xs font-medium rounded-sm text-signal-green border border-signal-green/30">
                          Voted
                        </span>
                      ) : (
                        <span className="px-2 py-1 text-xs font-medium rounded-sm text-signal-amber border border-signal-amber/30">
                          Pending Vote
                        </span>
                      )}
                      {dispute.deadline && (
