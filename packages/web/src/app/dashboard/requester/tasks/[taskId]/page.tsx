  ...
  if (!task) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/dashboard/requester" className="text-field-500 hover:text-field-600 mt-4 inline-block">
          Back to tasks
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <div className="flex justify-between items-start mb-8">
        <div>
          <Link href="/dashboard/requester" className="text-sm text-ink-500 hover:text-ink-700 mb-2 inline-block">
            &larr; Back to tasks
          </Link>
          <h1 className="text-2xl font-bold text-ink-900 tracking-tight">{task.title}</h1>
          <div className="flex items-center mt-2 space-x-3">
            <span className={`px-2 py-1 text-xs rounded-sm ${statusColors[task.status]}`}>
              {task.status}
            </span>
            <span className="text-sm text-ink-500">{task.template}</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-6">
          <div className="bg-paper rounded-sm border border-ink-200 p-6">
            <h2 className="text-lg font-medium text-ink-900 mb-4">Instructions</h2>
            <p className="text-ink-700 whitespace-pre-wrap">{task.instructions}</p>
          </div>

          <div className="bg-paper rounded-sm border border-ink-200 p-6">
            <h2 className="text-lg font-medium text-ink-900 mb-4">Requirements</h2>
            <dl className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <dt className="text-xs uppercase tracking-wider text-ink-500">Photos Required</dt>
                <dd className="text-ink-900 font-medium font-mono tabular-nums">{task.requirements?.photos?.count || '-'}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wider text-ink-500">Min Resolution</dt>
                <dd className="text-ink-900 font-mono tabular-nums">{task.requirements?.photos?.min_width_px}x{task.requirements?.photos?.min_height_px}</dd>
              </div>
              {task.requirements?.bearing?.required && (
                <>
                  <div>
                    <dt className="text-xs uppercase tracking-wider text-ink-500">Target Bearing</dt>
                    <dd className="text-ink-900 font-mono tabular-nums">{task.requirements.bearing.target_deg}&deg;</dd>
                  </div>
                  <div>
                    <dt className="text-xs uppercase tracking-wider text-ink-500">Bearing Tolerance</dt>
                    <dd className="text-ink-900 font-mono tabular-nums">&plusmn;{task.requirements.bearing.tolerance_deg}&deg;</dd>
                  </div>
                </>
              )}
            </dl>
        ...
        <div className="space-y-6">
                ...
                <div className="p-3 bg-ink-50 border-t border-ink-200">
                  <div className="flex items-center gap-4 text-xs">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-ink-500">Task center</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="w-2.5 h-2.5 rounded-sm bg-[#6366f1]" />
                      <span className="text-ink-500">Submission</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="w-2.5 h-2.5 rounded-sm bg-[#22c55e]" />
                      <span className="text-ink-500">Accepted</span>
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="bg-paper rounded-sm border border-ink-200 p-6">
            <h2 className="text-lg font-medium text-ink-900 mb-4">Details</h2>
            <dl className="space-y-3 text-sm">
              <div>
                <dt className="text-xs uppercase tracking-wider text-ink-500">Bounty</dt>
                <dd className="text-xl font-bold font-mono tabular-nums text-ink-900">
                  {task.bounty?.currency} {task.bounty?.amount?.toFixed(2)}
                </dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wider text-ink-500">Time Window</dt>
              ...
              <div>
                <dt className="text-xs uppercase tracking-wider text-ink-500">Exclusivity</dt>
                <dd className="text-ink-900 font-mono tabular-nums">{task.rights?.exclusivity_days} days</dd>
              </div>
            </dl>
          </div>

          {task.policy?.safety_notes && (
            <div className="bg-signal-amber/10 border border-signal-amber/30 rounded-sm p-4">
              <h3 className="text-sm font-medium text-ink-900 mb-2">Safety Notes</h3>
