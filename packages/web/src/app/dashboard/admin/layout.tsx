    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-ink-500">You don&apos;t have permission to access this area.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-6">
      ...
      <aside className="w-64 flex-shrink-0">
        <div className="bg-paper rounded-sm border border-ink-200 p-4 sticky top-24">
          <div className="mb-4 pb-4 border-b border-ink-200">
            <h2 className="text-lg font-semibold text-ink-900">Admin Panel</h2>
            <p className="text-xs text-ink-500 mt-1">Manage platform operations</p>
          </div>
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-sm text-sm font-medium transition-colors ${
                  isActive(item.href)
                    ? 'bg-field-50 text-field-500 border border-field-200'
                    : 'text-ink-500 hover:bg-paper-warm hover:text-ink-900'
                }`}
              >
                <svg
                  className={`w-5 h-5 ${isActive(item.href) ? 'text-field-500' : 'text-ink-300'}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          <div className="mt-6 pt-4 border-t border-ink-200">
            <div className="text-xs uppercase tracking-wider text-ink-500 mb-2">Session Info</div>
            <div className="text-xs text-ink-300">
              Logged in as <span className="text-ink-700">{user?.email || 'Admin'}</span>
            </div>
          </div>
        </div>
      </aside>
